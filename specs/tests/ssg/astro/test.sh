#!/usr/bin/env bash
# tests/ssg/astro/test.sh — Smoke test: Astro static site generation.
#
# Verifies that an Astro project can be installed and built inside
...
cp "$FIXTURE_DIR/astro.config.mjs"      "$TEST_DIR/"
mkdir -p "$TEST_DIR/src/pages"
cp "$FIXTURE_DIR/src/pages/index.astro" "$TEST_DIR/src/pages/"
...
_info "Step 2/2  : astro build"
_step _dxbx_exec npm run build

_assert_exists "dist/index.html" "dist/index.html"
